...
let existingHistory = JSON.parse(localStorage.getItem("history-tasks")) || [];

// Filter out tasks that are already in history (by id or text)
incompleteTasks.forEach(task => {
  let alreadyExists = existingHistory.some(ht => ht.id === task.id || ht.text === task.text);
  if (!alreadyExists) {
    existingHistory.push(task);
  }
});

localStorage.setItem("history-tasks", JSON.stringify(existingHistory));
...
if (updated.length > maxHistoryLength) {
  updated.splice(0, updated.length - maxHistoryLength);
}
    // Clear all tasks
...
viewHistoryTasksBtn.addEventListener("click", () => {
  ...
  list.innerHTML = "";
  history.forEach((entry, dateIndex) => {
    const dateHeading = document.createElement("li");
    dateHeading.innerHTML = `<strong>${entry.date}</strong>`;
    list.appendChild(dateHeading);

    entry.tasks.forEach((task, taskIndex) => {
      const li = document.createElement("li");
      li.innerHTML = `
        <input type="checkbox" onchange="completeTaskFromHistory(${dateIndex}, ${taskIndex})" />
        <span>${task.text}</span>
      `;
      list.appendChild(li);
    });
...
const existingHistory = JSON.parse(localStorage.getItem("todo-history")) || [];

// Only add the new task if it doesn't already exist in history
const isDuplicate = existingHistory.some(h => h.text === newTask.text && !h.completed);
if (!isDuplicate) {
  const updatedHistory = [
    { ...newTask, deletedAt: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) },
    ...existingHistory
  ];

  // Limit to 50 most recent items
  const limitedHistory = updatedHistory.slice(0, 50);
  localStorage.setItem("todo-history", JSON.stringify(limitedHistory));
}
...
window.completeTaskFromHistory = (dateIndex, taskIndex) => {
  const history = JSON.parse(localStorage.getItem("todo-history")) || [];
  if (history[dateIndex] && history[dateIndex].tasks[taskIndex]) {
    history[dateIndex].tasks.splice(taskIndex, 1);
    if (history[dateIndex].tasks.length === 0) {
      history.splice(dateIndex, 1); // Remove entire date if no tasks left
    }
    localStorage.setItem("todo-history", JSON.stringify(history));
    viewHistoryTasksBtn.click(); // Reload updated list
  }
};
...
document.getElementById("submitDevPass").addEventListener("click", () => {
  const entered = document.getElementById("devPassword").value;
  if (entered === "12345") {
    localStorage.setItem("developerMode", "true");
    alert("Developer mode enabled. Please go to the main page to access Force Reset.");
    document.getElementById("developerPanel").style.display = "none";
  } else {
...
// Show Force Reset only if developerMode is true
if (localStorage.getItem("developerMode") === "true") {
  document.getElementById("forceResetBtn").style.display = "inline-block";
}
...
// ✅ Show uncompleted tasks page when View Tasks button is clicked
document.getElementById("viewHistoryTasksBtn").addEventListener("click", () => {
  const page = document.getElementById("historyTasksPage");
  const list = document.getElementById("viewTaskList");
  const deleteBtn = document.getElementById("deleteAllBtn");
...
// ✅ Mark task complete from history
window.completeTaskFromHistory = (entryIndex, taskIndex) => {
  const history = JSON.parse(localStorage.getItem("todo-history")) || [];

  if (history[entryIndex] && history[entryIndex].tasks[taskIndex]) {
    history[entryIndex].tasks.splice(taskIndex, 1); // remove task

    if (history[entryIndex].tasks.length === 0) {
      history.splice(entryIndex, 1); // remove empty date section
    }

    localStorage.setItem("todo-history", JSON.stringify(history));
    document.getElementById("viewHistoryTasksBtn").click(); // Refresh view
  }
};
...
// ✅ Delete all from history when button clicked
document.getElementById("deleteAllBtn").addEventListener("click", () => {
  if (confirm("Are you sure you want to delete all uncompleted tasks?")) {
    localStorage.removeItem("todo-history");
    document.getElementById("viewTaskList").innerHTML = "";
    alert("All uncompleted tasks deleted.");
    closeViewTaskPage();
  }
});
